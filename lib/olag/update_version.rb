module Olag
  ...
  module Version
    ...
    # contain a line in the format: +return+ "+major+.+minor+.+commits+".
    # The third number is updated according to the number of Git commits. This
    # works well as long as we are working in the master branch.
  ...
  protected
    ...
    def self.extract_version(path, file_contents)
      abort("#{path}: Does not contain a valid VERSION line.") unless file_contents =~ /return ["'](\d+\.\d+\.)(\d+)["']/
      return [ $1, $2 ]
    end
